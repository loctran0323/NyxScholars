...
type Phase = "prior" | "welcome" | "calibration" | "running" | "paused" | "results";

const TOTAL = 14;

export default function DiagnosticClient({ existingSummary }: { existingSummary: DiagnosticSummary | null }) {
  const [phase, setPhase] = useState<Phase>(existingSummary ? "prior" : "welcome");
  ...
  return (
    ...
    >
      ...
      {phase === "prior" && existingSummary && (
        <PriorResults summary={existingSummary} onRetake={startRetake} />
      )}
      {phase === "welcome" && <Welcome onStart={() => setPhase("calibration")} />}
      {phase === "calibration" && <Calibration onContinue={() => setPhase("running")} />}
      {phase === "running" && current && (
        <Runner q={current} questionIndex={questionIndex} onAnswer={recordAnswer} />
      )}
      {phase === "paused" && <Paused onResume={() => setPhase("running")} />}
      {phase === "results" && (
        <Results theta={state.theta} ci={state.ci} answers={answers} onRestart={startRetake} />
      )}
    </div>
  );
}
...
/* ───────────────────────────────────────────────────────────
 * Prior Results — shown when the user already has a saved intake
 * ─────────────────────────────────────────────────────────── */
function PriorResults({ summary, onRetake }: { summary: DiagnosticSummary; onRetake: () => void }) {
  const completedDate = new Date(summary.completed_at).toLocaleDateString("en-US", {
    month: "long", day: "numeric", year: "numeric",
  });

  const ciScore = Math.round(summary.ci * 60);

  // Build skill rows from per_skill + ALL_SKILLS name lookup, sorted by mastery
  const skillRows = useMemo(() => {
    return Object.entries(summary.per_skill ?? {})
      .map(([id, mastery]) => {
        const meta = ALL_SKILLS.find((s) => s.id === id);
        return { id, name: meta?.name ?? id, mastery };
      })
  ...
  return (
    <div className="absolute inset-0 pt-20 pb-12 flex flex-col overflow-y-auto">
      <div
        className="flex-none text-center px-6 sm:px-8 pt-10 pb-6"
        style={{ opacity: revealStep >= 1 ? 1 : 0, transform: `translateY(${revealStep >= 1 ? 0 : 10}px)`, transition: "all 0.6s ease-out" }}
      >
        <div className="font-mono" style={{ fontSize: 10, letterSpacing: 6, color: MOON }}>
          YOUR INTAKE · {completedDate.toUpperCase()}
        </div>
        <h1
          className="font-light mt-4 mb-0 leading-[1.05]"
          style={{ fontFamily: "var(--font-fraunces)", fontSize: "clamp(2.4rem, 5vw, 3.8rem)", color: TEXT }}
        >
          Estimated range{" "}
          <span style={{ fontStyle: "italic", color: MOON }}>~{summary.predicted_score}</span>
        </h1>
        <p className="mt-3" style={{ fontSize: 14, color: TEXT_DIM }}>
          From {summary.questions} questions · ±{ciScore} points confidence interval
        </p>
      </div>

      {/* Skill breakdown */}
      <div
        className="flex-none px-6 sm:px-8 mx-auto w-full"
        style={{
          maxWidth: 820,
          opacity: revealStep >= 2 ? 1 : 0,
          transform: `translateY(${revealStep >= 2 ? 0 : 10}px)`,
      ...
      >
        ...
        >
          <div className="font-mono mb-5" style={{ fontSize: 9, letterSpacing: 4, color: TEXT_DIM }}>
            SKILL BREAKDOWN · {skillRows.length} SKILLS MEASURED
          </div>
          <div className="grid sm:grid-cols-2 gap-x-8 gap-y-4">
            {skillRows.map(({ id, name, mastery }) => {
              const color = mastery > 0.7 ? MOON_HI : mastery > 0.4 ? MOON : MOON_DIM;
              return (
                <div key={id}>
                  <div className="flex justify-between mb-1.5">
                    <span style={{ fontSize: 12, color: TEXT }}>{name}</span>
                    <span className="font-mono tabular-nums" style={{ fontSize: 11, color }}>
                      {Math.round(mastery * 100)}%
                    </span>
                  </div>
                  <div className="h-1 rounded-full overflow-hidden" style={{ background: NIGHT_3 }}>
                    <div
                      className="h-full rounded-full"
                      style={{
                        width: `${mastery * 100}%`,
                        background: color,
                        boxShadow: mastery > 0.4 ? `0 0 6px ${color}88` : "none",
      ...
      {/* Actions */}
      <div
        className="flex-none mt-8 px-6 sm:px-8 flex flex-wrap justify-center gap-3"
        style={{
      ...
      >
        <Link
          ...
          className="font-mono font-medium hover:brightness-110 transition-all"
          style={{
            padding: "14px 32px",
            background: MOON,
            color: NIGHT,
        ...
        <button
          onClick={onRetake}
          className="font-mono cursor-pointer hover:border-[#3b7a99] transition-colors"
          style={{
            padding: "14px 24px",
...
function TopChrome({
  ...
  if (phase === "prior" || phase === "welcome" || phase === "paused") {
    return (
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-1.5">
        ...
        <span className="ml-3 font-mono tabular-nums" style={{ fontSize: 10, letterSpacing: 2, color: TEXT_DIM }}>
...
function Logo() {
  return (
    <div className="flex items-center gap-2.5">
      <NyxMark size={22} />
      <div className="italic" style={{ color: TEXT, fontFamily: "var(--font-fraunces)", fontSize: 17 }}>
        Nyx
      </div>
    </div>
  );
  ...
  return (
    <div className="flex items-center gap-3.5">
      <div className="text-right">
        ...
        <div className="mt-0.5 font-mono tabular-nums" style={{ fontSize: 11, color: MOON, letterSpacing: 1 }}>
    ...
    <div className="absolute inset-0 flex items-center justify-center px-6 overflow-y-auto">
      <div className="max-w-[640px] py-24 text-center">
        ...
        <h1
          ...
          style={{ fontFamily: "var(--font-fraunces)", fontSize: "clamp(2.4rem, 5vw, 3.6rem)", color: TEXT }}
        >
...
/* ───────────────────────────────────────────────────────────
 * Results — score reveal + matched tutors
 * ─────────────────────────────────────────────────────────── */
function Results({
  ...
  return (
    <div className="absolute inset-0 pt-20 pb-12 flex flex-col overflow-y-auto">
      <div className="flex-none text-center px-6 sm:px-8 pt-10 pb-5">
        ...
        <p
          className="mt-3"
          style={{ fontSize: 14, color: TEXT_DIM, opacity: revealStep >= 1 ? 1 : 0, transition: "opacity 0.7s 0.2s" }}
        >
    ...
    >
      ...
      <div className="mt-auto pt-3" style={{ borderTop: `1px solid ${LINE}` }}>
        ...
        <Link
          ...
          style={{ padding: 12, background: MOON, color: NIGHT, fontSize: 11, letterSpacing: 4, borderRadius: 3 }}
        >
          BOOK FREE TRIAL →
        </Link>
    ...
    >
      ...
      <svg width="100%" height="100%" viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="xMidYMid slice">
        ...
        {positions.map((p, i) => {
          ...
          return (
            <g key={i} opacity={lit ? 1 : 0} style={{ transition: `opacity 0.5s ${0.2 + i * 0.12}s` }}>
